...
# Select device
deviceType = "/cpu:0"
tfConfig = tf.ConfigProto(intra_op_parallelism_threads=40, inter_op_parallelism_threads=5, allow_soft_placement=True, device_count = {'CPU': 10})
...
def complexModel():
    with tf.device(deviceType):
        ...
        fully_connected_output = tf.matmul(hFlat, W1) + b_fully_h
        #       Relu Activation
        ...
        W2 = tf.get_variable("w", shape=[1024,10])
        b = tf.get_variable("b", shape=[10])
        # - Store last layer output in yOut
        yOut = tf.matmul(relu_outFlat, W2) + b
